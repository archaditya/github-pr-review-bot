...
/**
 * The steps here are the *actual* ReviewJob state machine (docs/architecture/data-model.md
 * / apps/api/src/constants/review-job-status.js) — not decorative numbering. RETRYING
 * doesn't map to a fixed step since it can re-enter the pipeline at any stage; it's shown
 * as "somewhere in progress" rather than pinned to one step.
 */
const STEPS: { key: ReviewJobStatus; label: string }[] = [
  { key: 'PENDING', label: 'Queued' },
  { key: 'FETCHING_DIFF', label: 'Fetch diff' },
  { key: 'RESOLVING_USAGES', label: 'Resolve usages' },
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <span
                className={cn(
                  ...
                  isPast || isCurrent || isFailedHere ? 'text-foreground' : 'text-muted-foreground',
                )}
              >
